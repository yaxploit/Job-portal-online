...
export default function EditJobModal({ isOpen, onClose, onSaveJob, job }: EditJobModalProps) {
  ...
  // Load job data when the modal opens
  useEffect(() => {
    if (isOpen && job) {
      setTitle(job.title || "");
      ...
      // Extract min and max salaries from the salary string if needed
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Create updated job object (keeping original ID and other metadata)
    const updatedJob = {
      ...job,
      title,
      company,
